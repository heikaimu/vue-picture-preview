<template>
  <div class="big-picture-preview">
    ...
    <div class="original-wrapper" ref="imgWrapper">
      <img :class="{'show':isShowPic}" :src="currentPicSrc" :style="{'transition': `${animateSec}s`, 'transform': `rotate(${imgObj.rotate}deg) scale(${imgObj.scale})`, 'left': `${imgObj.left}px`, 'top': `${imgObj.top}px`}" ref="img" @load="imageLoaded" @mousedown="touchStart" @mousemove="touchMove" @mouseup="touchEnd" @mousewheel="mousewheel" @click="doubleClick">
    ...
    <div class="float-footer">
      ...
      <div class="thumb-wrapper" ref="thumbWrapper">
        <ul :style="{width: thumbWidth, marginLeft: thumbMarginLeft}">
          <li v-for="(item, index) in thumbnailList" :key="index" :style="{backgroundImage: `url(${item})`}" :class="{active: currentIndex===index}" @click="changePic(index)"></li>
        </ul>
      </div>
    </div>
    <!-- 左右侧按钮 -->
